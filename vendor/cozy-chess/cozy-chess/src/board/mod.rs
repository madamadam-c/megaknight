...
/// Information already known about a generated legal move.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct MoveMetadata {
    /// The piece being moved.
    pub piece: Piece,
    /// The move's effect on board occupancy.
    pub kind: MoveKind
}

/// The occupancy-changing category of a legal move.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum MoveKind {
    /// A move to an empty square.
    Quiet,
    /// A capture on the move's destination square.
    Capture(Piece),
    /// An en-passant capture.
    EnPassant,
    /// A castling move encoded as the king capturing its rook.
    Castle
}
...
impl Board {
    ...
    /// Get the incrementally updated hash of the pawn positions for `color`.
    #[inline(always)]
    pub fn pawn_hash(&self, color: Color) -> u64 {
        self.inner.pawn_hash(color)
    }

    /// Get the incrementally updated hash of all non-pawn piece positions for `color`.
    ...
    /// Get the incrementally updated hash of knight and bishop positions for `color`.
    #[inline(always)]
    pub fn minor_piece_hash(&self, color: Color) -> u64 {
        self.inner.minor_piece_hash(color)
    }

    /// Get the incrementally updated hash of rook and queen positions for `color`.
    #[inline(always)]
    pub fn major_piece_hash(&self, color: Color) -> u64 {
        self.inner.major_piece_hash(color)
    }
    ...
    pub fn pinned(&self) -> BitBoard {
        self.pinned_for(self.side_to_move())
    }

    /// Get the pieces pinned to the king of `color`.
    /// Note that this counts pieces regardless of their color.
    /// This counts any piece preventing check on that king.
    #[inline(always)]
    pub fn pinned_for(&self, color: Color) -> BitBoard {
    ...
    pub fn play_unchecked(&mut self, mv: Move) {
        ...
        self.play_unchecked_with_piece(mv, moved);
    }

    /// Unchecked version of [`Board::play_unchecked`] for callers that already
    /// know which piece is moving.
    ///
    /// Supplying the wrong piece may corrupt the board state. Only legal moves
    /// and their corresponding pieces should be passed.
    #[inline]
    pub fn play_unchecked_with_piece(&mut self, mv: Move, moved: Piece) {
        let color = self.inner.side_to_move();
        let is_castle = self.colors(color).has(mv.to);
        let victim = if is_castle || !self.colors(!color).has(mv.to) {
            None
        } else {
            self.piece_on(mv.to)
        };
        ...
        self.play_unchecked_known(mv, moved, victim, color, is_castle, is_en_passant);
    }

    /// Play a generated legal move using metadata already computed by the caller.
    ///
    /// Supplying incorrect metadata may corrupt the board state.
    #[inline]
    pub fn play_unchecked_with_metadata(&mut self, mv: Move, metadata: MoveMetadata) {
        match metadata.kind {
            MoveKind::Quiet => self.play_unchecked_quiet(mv, metadata.piece),
            MoveKind::Capture(victim) => {
                self.play_unchecked_capture(mv, metadata.piece, victim)
            }
            MoveKind::EnPassant => self.play_unchecked_en_passant(mv),
            MoveKind::Castle => self.play_unchecked_castle(mv)
        }
    }

    /// Play a generated legal non-capture using its known moved piece.
    #[inline]
    pub fn play_unchecked_quiet(&mut self, mv: Move, moved: Piece) {
        let color = self.inner.side_to_move();
        if moved == Piece::Pawn {
            self.play_unchecked_known(mv, Piece::Pawn, None, color, false, false);
        } else {
            self.play_unchecked_known(mv, moved, None, color, false, false);
        }
    }

    /// Play a generated legal capture using its known moved and captured pieces.
    #[inline]
    pub fn play_unchecked_capture(&mut self, mv: Move, moved: Piece, victim: Piece) {
        self.play_unchecked_known(
            mv,
            moved,
            Some(victim),
            self.inner.side_to_move(),
    ...
    /// Play a generated legal en-passant capture.
    #[inline]
    pub fn play_unchecked_en_passant(&mut self, mv: Move) {
        self.play_unchecked_known(
            mv,
            Piece::Pawn,
            None,
    ...
    /// Play a generated legal castling move.
    #[inline]
    pub fn play_unchecked_castle(&mut self, mv: Move) {
        self.play_unchecked_known(
            mv,
            Piece::King,
            None,
    ...
    fn play_unchecked_known(
        ...
        if is_castle {
            ...
            // Lift the piece, drop it on its destination.
            self.inner.move_square(moved, color, mv.from, mv.to);
        ...
        // Update checker and pin information for sliding pieces.
        ...
        for square in our_attackers {
            ...
            if between.is_empty() {
                self.checkers |= square.bitboard();
            } else if between.0 & (between.0 - 1) == 0 {
                self.pinned[(!color) as usize] |= between;
            }
        }

        // The previous side's pins are not needed for move generation, but are
        // cached for clients such as SEE.
        self.pinned[color as usize] = self.calculate_checkers_and_pins(color).1;
...
mod tests {
    ...
    fn play_moves() {
        ...
        for &(mv, expected) in MOVES {
            ...
            assert_eq!(format!("{}", board), expected);
            let expected = expected.parse::<Board>().unwrap();
            assert_eq!(board.hash(), expected.hash());
            for color in Color::ALL {
                assert_eq!(board.pinned_for(color), expected.pinned_for(color));
                assert_eq!(board.pawn_hash(color), expected.pawn_hash(color));
                assert_eq!(board.non_pawn_hash(color), expected.non_pawn_hash(color));
                assert_eq!(board.minor_piece_hash(color), expected.minor_piece_hash(color));
                assert_eq!(board.major_piece_hash(color), expected.major_piece_hash(color));
            }
        }
    }

    #[test]
    fn tracks_pins_for_both_colors() {
        let mut board: Board = "3kr3/3p4/8/8/8/8/4P3/3RK3 w - - 0 1"
            .parse()
            .unwrap();

        assert_eq!(board.pinned(), Square::E2.bitboard());
        assert_eq!(board.pinned_for(Color::White), Square::E2.bitboard());
        assert_eq!(board.pinned_for(Color::Black), Square::D7.bitboard());

        let null_board = board.null_move().unwrap();
        assert_eq!(null_board.pinned(), Square::D7.bitboard());
        assert_eq!(null_board.pinned_for(Color::White), Square::E2.bitboard());
        assert_eq!(null_board.pinned_for(Color::Black), Square::D7.bitboard());

        board.play("e1f1".parse().unwrap());
        assert_eq!(board.pinned(), Square::D7.bitboard());
        assert_eq!(board.pinned_for(Color::White), BitBoard::EMPTY);
        assert_eq!(board.pinned_for(Color::Black), Square::D7.bitboard());
    }

...
impl KingSafety {
    #[inline(always)]
    fn safe_on(self, square: Square) -> bool {
        if !(get_pawn_attacks(square, self.color) & self.pawns).is_empty()
            || !(get_knight_moves(square) & self.knights).is_empty()
            || !(get_king_moves(square) & self.king).is_empty()
        {
            return false;
        }

        let blockers = self.blockers | square.bitboard();
        if !(get_bishop_rays(square) & self.diagonal_sliders).is_empty()
            && !(get_bishop_moves(square, blockers) & self.diagonal_sliders).is_empty()
        {
            return false;
        }

        (get_rook_rays(square) & self.orthogonal_sliders).is_empty()
            || (get_rook_moves(square, blockers) & self.orthogonal_sliders).is_empty()
    }
}
...
impl Board {
    ...
    >(&self, mask: BitBoard, listener: &mut F) -> bool {
        ...
        if let Some(en_passant) = self.en_passant() {
            ...
            let candidates = get_pawn_attacks(dest, !color) & pieces;
    ...
    #[inline(always)]
    fn king_safety(&self) -> KingSafety {
        let color = self.side_to_move();
        let their_pieces = self.colors(!color);
        KingSafety {
            color,
            blockers: self.occupied() ^ self.colored_pieces(color, Piece::King),
            pawns: their_pieces & self.pieces(Piece::Pawn),
            knights: their_pieces & self.pieces(Piece::Knight),
            king: their_pieces & self.pieces(Piece::King),
            diagonal_sliders: their_pieces
                & (self.pieces(Piece::Bishop) | self.pieces(Piece::Queen)),
            orthogonal_sliders: their_pieces
    ...
    fn king_safe_on(&self, square: Square) -> bool {
        self.king_safety().safe_on(square)
    }
    ...
    fn can_castle(
        ...
        !pinned.has(rook)
            ...
            && must_be_safe.iter().all(|square| safety.safe_on(square))
    }
    ...
    >(&self, mask: BitBoard, listener: &mut F) -> bool {
        ...
        if !IN_CHECK {
            ...
            if let Some(rook) = rights.short {
                if self.can_castle(rook, File::G, File::F, safety) {
                    moves |= Square::new(rook, back_rank).bitboard();
                }
            }
            if let Some(rook) = rights.long {
                if self.can_castle(rook, File::C, File::D, safety) {
                    moves |= Square::new(rook, back_rank).bitboard();
                }
            }
        }
    ...
    fn king_is_legal(&self, mv: Move) -> bool {
        if self.checkers.is_empty() {
            ...
            if let Some(rook) = castles.short {
                ...
                if rook_square == mv.to
                    && self.can_castle(rook, File::G, File::F, self.king_safety())
                {
                    return true;
                }
            }
            if let Some(rook) = castles.long {
                let rook_square = Square::new(rook, back_rank);
                if rook_square == mv.to
                    && self.can_castle(rook, File::C, File::D, self.king_safety())
                {
                    return true;
                }
            }

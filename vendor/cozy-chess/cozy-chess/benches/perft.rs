...
use cozy_chess::{Board, MoveKind, MoveMetadata, Piece};
...
pub fn criterion_benchmark(criterion: &mut Criterion) {
    ...
    let positions = POSITIONS.iter()
        .map(|pos| {
            ...
            board.generate_moves(|moves| {
                ...
                all_moves.extend(moves.into_iter().map(|mv| {
                    let kind = if our_pieces.has(mv.to) {
                        MoveKind::Castle
                    } else if their_pieces.has(mv.to) {
                        MoveKind::Capture(board.piece_on(mv.to).unwrap())
                    } else if piece == Piece::Pawn && mv.from.file() != mv.to.file() {
                        MoveKind::EnPassant
                    } else {
                        MoveKind::Quiet
                    };
                    (mv, piece, MoveMetadata { piece, kind })
                }));
                false
            });
            (board, all_moves)
    ...
    criterion
        ...
        .bench_function("Play moves with known piece", |b| {
            b.iter(|| {
                for (board, moves) in &positions {
                    for &(mv, piece, _) in moves {
                        let mut board = board.clone();
                        board.play_unchecked_with_piece(mv, piece);
                        black_box(board);
                    }
                }
            });
        })
        .bench_function("Play moves with metadata", |b| {
            b.iter(|| {
                for (board, moves) in &positions {
                    for &(mv, _, metadata) in moves {
                        let mut board = board.clone();
                        board.play_unchecked_with_metadata(mv, metadata);
                        black_box(board);
                    }
                }
            });
        })
        .bench_function("Play moves specialized", |b| {
            b.iter(|| {
                for (board, moves) in &positions {
                    for &(mv, _, metadata) in moves {
                        let mut board = board.clone();
                        match metadata.kind {
                            MoveKind::Quiet => board.play_unchecked_quiet(mv, metadata.piece),
                            MoveKind::Capture(victim) => {
                                board.play_unchecked_capture(mv, metadata.piece, victim)
                            }
                            MoveKind::EnPassant => board.play_unchecked_en_passant(mv),
                            MoveKind::Castle => board.play_unchecked_castle(mv)
                        }
                        black_box(board);
                    }
                }

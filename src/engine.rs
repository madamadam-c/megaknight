...
pub const FLAG_CAPTURE:    u8 = 2;
pub const FLAG_EN_PASSANT: u8 = 4;
pub const FLAG_PROMOTION:  u8 = 8;
pub const FLAG_TT_MOVE:    u8 = 16;
...
impl MovePicker {
    fn new(mut moves: MoveList) -> Self {
        let tt_move = moves
            .good_captures
            .iter()
            .position(|mv| mv.flags & FLAG_TT_MOVE != 0)
...
#[inline(always)]
fn play_move(board: &Board, mv: &EngineMove) -> Board {
    let mut next_board = board.clone();

    if mv.flags & FLAG_CASTLE != 0 {
        next_board.play_unchecked_castle(mv.mv);
    } else if mv.flags & FLAG_EN_PASSANT != 0 {
        next_board.play_unchecked_en_passant(mv.mv);
    } else if mv.flags & FLAG_CAPTURE != 0 {
        next_board.play_unchecked_capture(mv.mv, mv.piece_type, mv.target_type.unwrap());
    } else {
        next_board.play_unchecked_quiet(mv.mv, mv.piece_type);
    }
...
impl Engine {
    ...
    pub fn get_correction_value(&self, board: &Board) -> i32 {
        ...
        let correction = (0 // just for formatting reasons
            + 290 * self.pawn_correction_history.get(board.side_to_move() as usize, pawn_hash)
        ...
        //     + 0*self.major_correction_history.get(board.side_to_move() as usize, major_hash)
            + 398 * self.stm_non_pawn_correction_history.get(board.side_to_move() as usize, stm_non_pawn_hash)
            + 398 * self.nstm_non_pawn_correction_history.get(board.side_to_move() as usize, nstm_non_pawn_hash))
            / 16384;
        return correction;
    }
        ...
        board.generate_moves(|moves_for_piece| {
            for mv in moves_for_piece {
                ...
                if emv.flags & FLAG_CAPTURE != 0 {
                    ...
                    emv.history =
                        (1035 * quiet_history + 915 * pawn_history + 1114 * continuation_history)
                            / 1024;
                }

                if emv.flags & (FLAG_CAPTURE | FLAG_TT_MOVE) != 0{
                    if emv.see_score >= 0 || emv.flags & FLAG_TT_MOVE != 0 {
    ...
    fn generate_qsearch_moves(&self, board: &Board, empty_flag: &mut bool) -> MoveList {
        ...
        board.generate_moves(|mut moves_for_piece| {
            ...
            for mv in moves_for_piece {
                ...
                } else if emv.flags & FLAG_PROMOTION != 0 {
                    moves.quiets.push(emv);
                    emv.history =
                        self.quiet_history
                            .get(board.side_to_move() as usize, mv.from, mv.to);
    ...
    fn quiesce(
        ...
        while let Some(mv) = picker.next() {
            if !in_check && mv.see_score < -24 {
                continue;
            }
            let next_board = play_move(board, &mv);
    ...
    fn minimax(
                ...
                >= 1
        {
            let null_base = 4;
            let null_div = 3;
            let null_eval_div = 183;
            let null_max_extra_reduction = 3;
        ...
        while let Some(mv) = picker.next() {
            ...
            // lmp
            if !pv && !in_check && moves_played >= lmp_cap && mv.flags & (FLAG_CAPTURE | FLAG_PROMOTION) == 0 {
            ...
            // lmr
            let mut lmr_depth = 0;
            if depth >= 2 && moves_played >= 2 && mv.flags & (FLAG_CAPTURE | FLAG_PROMOTION) == 0 && !in_check {
                let lmr_base = 1018;
                let lmr_log_scale = 1806;

                // base formula [credit: obsidian on cpw]
                lmr_depth += ((lmr_base as f32)
                    + (lmr_log_scale as f32) * self.lmr_logs[depth as usize] * self.lmr_logs[moves_played as usize]
                        / 3.14) as i32;
                ...
                lmr_depth += 557 * !improving as i32;

                // reduce based on history (max reduction is +/- 2 plies)
                lmr_depth -= 1024 * mv.history / 5749;
            ...
            if x >= bounds.beta {
                // actual = NodeType::CUT;
                if mv.flags & (FLAG_CAPTURE | FLAG_PROMOTION) == 0 {
        ...
        if !in_check
            && best_move.is_none_or(|mv| mv.flags & (FLAG_CAPTURE | FLAG_PROMOTION) == 0)
            && !(node_type == TTNodeType::LOWER && result <= static_eval)
            && !(node_type == TTNodeType::UPPER && result >= static_eval)
            && result.abs() <= 95_000
        {
            let bonus = 339 * (result - static_eval) * depth / 1024;
            let pawn_hash =
                board.pawn_hash(board.side_to_move()) ^ board.pawn_hash(!board.side_to_move());
            // let minor_hash = board.minor_piece_hash(board.side_to_move()) ^ board.minor_piece_hash(!board.side_to_move());
            // let major_hash = board.major_piece_hash(board.side_to_move()) ^ board.major_piece_hash(!board.side_to_move());
        ...
        F: FnMut(SearchInfo),
    {
        ...
        let max_depth = request.limits.depth.unwrap_or(1024).max(1);

...
fn update_history(value: &mut i32, bonus: i32, scale: i32) {
    let bonus = bonus.clamp(-scale, scale);
    *value += bonus - *value * bonus.abs() / scale;
}

fn update_history_i16(value: &mut i16, bonus: i32, scale: i32) {
    let bonus = bonus.clamp(-scale, scale);
    let v = *value as i32;
...
impl QuietHistory {
    pub fn new() -> Self {
        Self {
            history: Box::new([[[0i32; 64]; 64]; 2])
        }
    }

    pub fn clear(&mut self) {
        self.history = Box::new([[[0i32; 64]; 64]; 2]);
    }

    pub fn get(&self, stm: usize, from: Square, to: Square) -> i32 {
        self.history[stm][from as usize][to as usize]
    }
...
impl ContinuationHistory {
    pub fn new() -> Self {
        Self {
            history: array::from_fn(|_| {Box::new([[[[[0i32; 6]; 64]; 6]; 64]; 2])})
        }
    }

    pub fn clear(&mut self) {
        self.history = array::from_fn(|_| {Box::new([[[[[0i32; 6]; 64]; 6]; 64]; 2])})
    }
...
impl CaptureHistory {
    pub fn new() -> Self {
        Self {
            history: Box::new([[[[0i32; 6]; 6]; 64]; 2])
        }
    }

    pub fn clear(&mut self) {
        self.history = Box::new([[[[0i32; 6]; 6]; 64]; 2])
    }

    pub fn get(&self, stm: usize, to: Square, piece: Piece, target: Piece) -> i32 {
        self.history[stm][to as usize][piece as usize][target as usize]
    }
